 ...
 * ============================================================================
 *  Turf.js  图形处理
 * 完整构建版 (turf.min.js) —— 功能速查手册
 *  包含模块：Turf核心 / JSTS拓扑 / RBush空间索引 / MarchingSquares / d3-voronoi
 *  坐标系：WGS84 [经度lng, 纬度lat]（注意顺序！）
 *  默认单位：kilometers
 * ============================================================================
 *
 * ┌─────────────────────────────────────────────────────────────────────────┐
 ...
 * │  六、变换与操作                                                          │
 * │  七、聚合与统计                                                          │
 * │  八、等值线 / 等值带（MarchingSquares）                                  │
 * │  九、Voronoi / Delaunay（d3-voronoi）                                   │
 * │  十、随机生成                                                            │
 * │  十一、辅助工具                                                          │
 * │  十二、与 Cesium 配合使用                                                │
 * │  十三、性能与注意事项                                                    │
 ...
 *
 * 一、创建几何对象
 * • 1.1 turf.point —— 创建点要素
 * • 1.2 turf.multiPoint —— 创建多点要素
 * • 1.3 turf.lineString —— 创建线要素
 * • 1.4 turf.multiLineString —— 创建多线要素
 * • 1.5 turf.polygon —— 创建多边形（支持孔洞）
 * • 1.6 turf.multiPolygon —— 创建多多边形
 * • 1.7 turf.featureCollection —— 创建要素集合
 * • 1.8 turf.circle —— 创建圆形多边形
 * • 1.9 turf.bboxPolygon —— bbox 转矩形多边形

 * 二、测量与计算
 * • 2.1 turf.distance —— 两点间大圆距离
 * • 2.2 turf.length —— 线或多边形周长
 * • 2.3 turf.area —— 多边形面积（㎡）
 * • 2.4 turf.bbox —— 计算外接矩形
 * • 2.5 turf.center —— 几何中心（bbox 中心）
 * • 2.6 turf.centroid —— 质心（面积加权）
 * • 2.7 turf.midpoint —— 两点中点
 * • 2.8 turf.destination —— 按距离与方位角推算终点
 * • 2.9 turf.bearing —— 起止点方位角
 * • 2.10 turf.rhumbDistance —— 等角航线距离
 * • 2.11 turf.rhumbBearing —— 等角航线方位角

 * 三、网格生成
 * • 3.1 turf.hexGrid —— 六边形蜂巢网格
 * • 3.2 turf.squareGrid —— 正方形网格
 * • 3.3 turf.triangleGrid —— 等边三角形网格
 * • 3.4 turf.pointGrid —— 规则点网格

 * 四、布尔运算（JSTS 拓扑引擎）
 * • 4.1 turf.union —— 多边形并集
 * • 4.2 turf.intersect —— 多边形交集
 * • 4.3 turf.difference —— 多边形差集
 * • 4.4 turf.buffer —— 缓冲区（点/线/面）
 * • 4.5 turf.dissolve —— 多边形合并（可按属性分组）

五、空间关系判断
 * • 5.1 turf.booleanPointInPolygon —— 点是否在多边形内
 * • 5.2 turf.booleanContains —— A 是否完全包含 B
 * • 5.3 turf.booleanWithin —— A 是否完全在 B 内
 * • 5.4 turf.booleanIntersects —— 是否存在交集
 * • 5.5 turf.booleanDisjoint —— 是否完全不相交
 * • 5.6 turf.booleanOverlap —— 是否部分重叠
 * • 5.7 turf.booleanEqual —— 几何是否完全一致
 * • 5.8 turf.booleanCrosses —— 是否交叉（线穿面/线）
 * • 5.9 turf.booleanTouch —— 是否仅边界接触
 * • 5.10 turf.lineIntersect —— 求两线交点集合
 * • 5.11 turf.pointOnFeature —— 获取几何内部一点

六、变换与操作

 * • 6.1 turf.transformTranslate —— 平移几何
 * • 6.2 turf.transformRotate —— 旋转几何
 * • 6.3 turf.transformScale —— 缩放几何
 * • 6.4 turf.simplify —— 几何简化（Douglas-Peucker）
 * • 6.5 turf.rewind —— 修正多边形环方向
 * • 6.6 turf.flip —— 交换经纬度顺序
 * • 6.7 turf.truncate —— 截断坐标小数位
 * • 6.8 turf.cleanCoords —— 清理冗余坐标
 * • 6.9 turf.lineSplit —— 切割线
 * • 6.10 turf.lineSlice —— 按点截取线段
 * • 6.11 turf.lineSliceAlong —— 按距离截取线段
 * • 6.12 turf.lineChunk —— 等分线段
 * • 6.13 turf.polygonToLine —— 多边形转边界线
 * • 6.14 turf.lineToPolygon —— 闭合线转多边形
 * • 6.15 turf.concave —— 生成凹包
 * • 6.16 turf.convex —— 生成凸包
 * • 6.17 turf.envelope —— 生成最小外接矩形

七、聚合与统计
 * • 7.1 turf.pointsWithinPolygon —— 筛选多边形内点
 * • 7.2 turf.collect —— 点属性聚合到多边形
 * • 7.3 turf.nearestPoint —— 查找最近点
 * • 7.4 turf.nearestPointOnLine —— 线上最近点
 * • 7.5 turf.tag —— 用多边形属性标记点
 * • 7.6 turf.clustersKmeans —— K-Means 聚类
 * • 7.7 turf.clustersDbscan —— DBSCAN 密度聚类

八、等值线 / 等值带（Marching Squares）
 * • 8.1 turf.isolines —— 生成等值线（线状）
 * • 8.2 turf.isobands —— 生成等值带（面状）
 * • 8.3 turf.interpolate —— 离散点插值网格（IDW）

九、Voronoi / Delaunay（d3-voronoi）
 * • 9.1 turf.voronoi —— 泰森多边形（Voronoi 图）
 * • 9.2 turf.tin —— Delaunay 三角剖分（TIN）

十、随机生成
 * • 10.1 turf.randomPosition —— 生成随机坐标
 * • 10.2 turf.randomPoint —— 生成随机点集合
 * • 10.3 turf.randomLineString —— 生成随机线
 * • 10.4 turf.randomPolygon —— 生成随机多边形

十一、辅助工具
 * • 11.1 turf.getCoord —— 提取单点坐标
 * • 11.2 turf.getCoords —— 提取任意几何坐标
 * • 11.3 turf.getGeom —— 提取 Geometry 对象
 * • 11.4 turf.getType —— 获取几何类型
 * • 11.5 turf.feature —— Geometry 转 Feature
 * • 11.6 turf.coordEach —— 遍历所有坐标
 * • 11.7 turf.featureEach —— 遍历所有要素
 * • 11.8 turf.flatten —— 拆分 Multi 几何
 * • 11.9 turf.explode —— 几何拆成独立点
 * • 11.10 turf.meta / turf.helpers —— 底层遍历与工具函数

十二、与 Cesium 配合使用
 * • 12.1 GeoJsonDataSource 直接加载 Turf 数据
 * • 12.2 手动坐标转换（Cartesian3）
 * • 12.3 等值线渲染（isolines + Cesium）
 * • 12.4 Voronoi 点击查询（booleanPointInPolygon）
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.1  turf.point(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建一个点要素
 *  参数：
 *    coordinates  {number[]}  [经度, 纬度, 高度?]
 *    properties   {Object}   附加属性（可选）
 *  返回：Feature<Point>
 *
 *  示例：
 *    var pt = turf.point([116.397, 39.908, 100]);
 *    var pt2 = turf.point([116.4, 39.9], { name: '北京' });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.2  turf.multiPoint(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建多点要素
 *  参数：
 *    coordinates  {number[][]}  [[lng,lat], [lng,lat], ...]
 *  返回：Feature<MultiPoint>
 *
 *  示例：
 *    var mpt = turf.multiPoint([[116.4,39.9],[117.2,40.1],[115.8,38.7]]);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.3  turf.lineString(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建线要素
 *  参数：
 *    coordinates  {number[][]}  至少2个坐标点
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.4  turf.multiLineString(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建多线要素
 *  参数：
 *    coordinates  {number[][][]}  [[[lng,lat],...], [[lng,lat],...]]
 *  返回：Feature<MultiLineString>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.5  turf.polygon(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建多边形（首尾坐标必须闭合！）
 *  参数：
 *    coordinates  {number[][][]}  第一个环=外环，后续环=洞
 *  返回：Feature<Polygon>
 *
 *  示例：
 ...
 *      [117.0, 40.0],
 *      [116.0, 40.0],
 *      [116.0, 39.0]    // ← 必须与第一个点相同（闭合）
 *    ]]);
 *
 *    // 带洞的多边形
 *    var polyWithHole = turf.polygon([
 *      [[0,0],[10,0],[10,10],[0,10],[0,0]],       // 外环
 *      [[3,3],[7,3],[7,7],[3,7],[3,3]]            // 洞
 *    ]);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.6  turf.multiPolygon(coordinates, properties?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：创建多多边形
 *  参数：
 *    coordinates  {number[][][][]}  多个 Polygon 的坐标数组
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.7  turf.featureCollection(features)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将多个要素打包为 FeatureCollection
 *  参数：
 *    features  {Feature[]}  要素数组
 *  返回：FeatureCollection
 *
 *  示例：
 *    var fc = turf.featureCollection([pt, line, poly]);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.8  turf.circle(center, radius, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：以某点为圆心生成近似圆形多边形
 *  参数：
 *    center   {Feature<Point>|number[]}  圆心
 *    radius   {number}                   半径
 *    options  {Object}                   { steps:64, units:'kilometers', properties:{} }
 *  返回：Feature<Polygon>
 *
 *  示例：
 *    var c = turf.circle([116.4, 39.9], 5, { steps: 128, units: 'kilometers' });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  1.9  turf.bboxPolygon(bbox)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将 bbox 转为矩形多边形
 *  参数：
 ...
 *
 *  示例：
 *    var rect = turf.bboxPolygon([116.0, 39.0, 117.0, 40.0]);
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  二、测量与计算
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.1  turf.distance(from, to, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算两点间的大地线距离（Haversine公式）
 *  参数：
 *    from/to  {Feature<Point>|number[]}
 *    options  { units: 'kilometers'|'miles'|'degrees'|'radians'|'meters' }
 *  返回：number
 *
 *  示例：
 *    var d = turf.distance([116.4,39.9], [117.2,40.1], { units: 'kilometers' });
 *    console.log(d);  // ≈ 85.3 km
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.2  turf.length(line, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算线/多线/多边形的总长度（周长）
 *  参数：
 *    line     {Feature<LineString|MultiLineString|Polygon>}
 *    options  { units }
 *  返回：number
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.3  turf.area(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算面积（平方米）
 *  参数：
 *    geojson  {Feature<Polygon|MultiPolygon>|FeatureCollection}
 *  返回：number（平方米）
 *
 *  示例：
 *    var a = turf.area(poly);  // 单位：m²
 *    console.log((a / 1e6).toFixed(2) + ' km²');
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.4  turf.bbox(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算任意 GeoJSON 的外接矩形
 *  返回：number[]  [minLng, minLat, maxLng, maxLat]
 *
 *  示例：
 *    var box = turf.bbox(featureCollection);
 *    // → [116.0, 39.0, 117.0, 40.0]
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.5  turf.center(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算几何中心（bbox中心）
 *  返回：Feature<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.6  turf.centroid(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算质心（面积加权中心）
 *  返回：Feature<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.7  turf.midpoint(pt1, pt2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：两点间的中点
 *  返回：Feature<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.8  turf.destination(origin, distance, bearing, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：从起点沿方位角走指定距离，求终点
 *  参数：
 *    origin    {Feature<Point>|number[]}
 *    distance  {number}   距离
 *    bearing   {number}   方位角（度，0=正北，90=正东）
 *    options   { units }
 *  返回：Feature<Point>
 *
 *  示例：
 *    // 从北京向东北45°走100km
 *    var dest = turf.destination([116.4,39.9], 100, 45, { units:'kilometers' });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.9  turf.bearing(start, end)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：计算从 start 到 end 的方位角
 *  返回：number（-180 ~ 180，正=顺时针从北）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.10  turf.rhumbDistance(from, to, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：等角航线距离（航海常用，非大圆距离）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  2.11  turf.rhumbBearing(from, to)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：等角航线方位角
 *
 *
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  3.1  turf.hexGrid(bbox, cellSide, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：在矩形范围内生成六边形蜂巢网格
 *  参数：
 *    bbox      {number[]}  [minLng, minLat, maxLng, maxLat]
 *    cellSide  {number}    六边形中心到顶点的距离（半径）
 *    options   {Object}
 *      .units       {string}   默认 'kilometers'
 *      .properties  {Object}   每个六边形的属性
 *      .mask        {Feature<Polygon|MultiPolygon>}  裁剪蒙版
 *      .triangles   {boolean}  true=输出三角形而非六边形
 *  返回：FeatureCollection<Polygon>
 *
 *  示例：
 *    var grid = turf.hexGrid([116,39,117,40], 5, {
 *      units: 'kilometers',
 *      mask: somePolygon   // 可选：只保留多边形内的格子
 *    });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  3.2  turf.squareGrid(bbox, cellSide, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成正方形网格
 *  参数：同 hexGrid（无 triangles 选项）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  3.3  turf.triangleGrid(bbox, cellSide, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成等边三角形网格
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  3.4  turf.pointGrid(bbox, cellSide, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成规则点网格
 *  参数：
 *    options.mask  可选裁剪
 *  返回：FeatureCollection<Point>
 *
 *  示例：
 *    var pts = turf.pointGrid([116,39,117,40], 10, { units:'kilometers' });
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  四、布尔运算（底层：JSTS 拓扑引擎）
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  原理：JSTS 通过 PlanarGraph → EdgeRing → Depth计算 → 结果边提取
 *        实现精确的多边形拓扑运算（非像素级，是数学精确）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  4.1  turf.union(polygon1, polygon2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：合并两个多边形（并集）
 *  参数：两个 Feature<Polygon> 或 Feature<MultiPolygon>
 *  返回：Feature<Polygon|MultiPolygon> 或 null（无交集时）
 *
 *  示例：
 *    var merged = turf.union(poly1, poly2);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  4.2  turf.intersect(polygon1, polygon2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：取两个多边形的交集
 *  返回：Feature<Polygon> 或 null
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  4.3  turf.difference(polygon1, polygon2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：从 polygon1 中减去 polygon2（差集）
 *  返回：Feature<Polygon|MultiPolygon> 或 null
 *
 *  示例：
 *    var hole = turf.difference(bigPoly, smallPoly);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  4.4  turf.buffer(geojson, radius, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成缓冲区（点→圆，线→带状，面→外扩）
 *  参数：
 *    geojson  {Feature|FeatureCollection|Geometry}
 *    radius   {number}  缓冲距离（负值=内缩）
 *    options  { units, steps:64 }
 *  返回：Feature<Polygon|MultiPolygon>
 *
 *  示例：
 *    var buf = turf.buffer(line, 500, { units: 'meters', steps: 128 });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  4.5  turf.dissolve(featureCollection, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将多个相邻/重叠多边形合并为一个（按属性分组）
 *  参数：
 *    options.propertyName  {string}  按此属性值分组合并
 *  返回：FeatureCollection<MultiPolygon>
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  五、空间关系判断
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  底层：RayCrossingCounter（射线法）判断点在内/外
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.1  turf.booleanPointInPolygon(point, polygon, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断点是否在多边形内
 *  参数：
 *    options.ignoreBoundary  {boolean}  true=边界上算外部
 *  返回：boolean
 *
 *  示例：
 *    var inside = turf.booleanPointInPolygon([116.5,39.5], poly);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.2  turf.booleanContains(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断 geojson1 是否完全包含 geojson2
 *  返回：boolean
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.3  turf.booleanWithin(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断 geojson1 是否完全在 geojson2 内部
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.4  turf.booleanIntersects(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断两个几何是否有任何交集（相交/包含/接触）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.5  turf.booleanDisjoint(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断两个几何是否完全不相交
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.6  turf.booleanOverlap(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断是否部分重叠（非包含关系）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.7  turf.booleanEqual(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断两个几何是否形状相同
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.8  turf.booleanCrosses(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断是否交叉（线穿面、线穿线）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.9  turf.booleanTouch(geojson1, geojson2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：判断是否仅边界接触（内部不重叠）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.10  turf.lineIntersect(line1, line2)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：求两条线的所有交点
 *  返回：FeatureCollection<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  5.11  turf.pointOnFeature(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：在几何内部找一个保证在内部的点
 *  返回：Feature<Point>
 *
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.1  turf.transformTranslate(geojson, distance, direction, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：平移几何
 *  参数：
 *    direction  {number}  方位角（度）
 *    options.mutate  {boolean}  true=直接修改原对象（更快）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.2  turf.transformRotate(geojson, angle, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：绕质心旋转
 *  参数：
 *    angle  {number}  旋转角度（正=逆时针）
 *    options.pivot  {Feature<Point>}  旋转中心（默认质心）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.3  turf.transformScale(geojson, factor, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：缩放几何
 *  参数：
 *    factor  {number}  >1放大，<1缩小
 *    options.origin  {string|Feature<Point>}  'sw'|'se'|'nw'|'ne'|'center'|'centroid'
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.4  turf.simplify(geojson, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：简化几何（减少顶点数，Douglas-Peucker算法）
 *  参数：
 *    options.tolerance   {number}   容差（默认1）
 *    options.highQuality {boolean}  true=更精确但更慢
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.5  turf.rewind(geojson, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：修正多边形环的方向（RFC7946：外环逆时针，洞顺时针）
 *  参数：
 *    options.reverse  {boolean}  true=反转默认方向
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.6  turf.flip(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：交换所有坐标的 [lng,lat] → [lat,lng]
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.7  turf.truncate(geojson, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：截断坐标小数位
 *  参数：
 *    options.precision  {number}  小数位数（默认6）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.8  turf.cleanCoords(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：移除冗余坐标（共线点、重复点）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.9  turf.lineSplit(line, splitter)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：用点/线/多边形切割线
 *  返回：FeatureCollection<LineString>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.10  turf.lineSlice(startPt, stopPt, line)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：截取线上两点之间的片段
 *  返回：Feature<LineString>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.11  turf.lineSliceAlong(line, startDist, stopDist, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：按距离截取线段
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.12  turf.lineChunk(line, chunkLength, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将线等分为多段
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.13  turf.polygonToLine(polygon)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：多边形边界 → 线
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.14  turf.lineToPolygon(line, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：闭合线 → 多边形
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.15  turf.concave(featureCollection, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成凹包（concave hull）
 *  参数：
 *    options.maxEdge  {number}  最大边长
 *    options.units    {string}
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.16  turf.convex(featureCollection)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成凸包（convex hull）
 *  返回：Feature<Polygon>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  6.17  turf.envelope(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成外接矩形多边形
 *
 *
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.1  turf.pointsWithinPolygon(points, polygons)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：筛选落在多边形内的所有点
 *  返回：FeatureCollection<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.2  turf.collect(polygonFC, pointFC, inField, outField)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将点的属性聚合到包含它的多边形上
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.3  turf.nearestPoint(targetPoint, pointsFC)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：找最近的点
 *  返回：Feature<Point>（附带 .properties.distanceToPoint）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.4  turf.nearestPointOnLine(line, point, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：找线上离目标点最近的位置
 *  返回：Feature<Point>（附带 .dist, .location）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.5  turf.tag(pointsFC, polygonsFC, field, outField)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：用多边形属性标记点
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.6  turf.clustersKmeans(points, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：K-Means 聚类
 *  参数：
 *    options.numberOfClusters  {number}  簇数
 *  返回：FeatureCollection（每个点带 .properties.cluster）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  7.7  turf.clustersDbscan(points, maxDistance, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：DBSCAN 密度聚类
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  八、等值线 / 等值带（底层：MarchingSquaresJS）
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  原理：Marching Squares 算法，256种case查表 + 线性插值
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  8.1  turf.isolines(pointGrid, zProperty, breaks, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：从带属性的点网格生成等值线
 *  参数：
 *    pointGrid  {FeatureCollection<Point>}  必须有 z 属性
 *    zProperty  {string}   属性名（如 'elevation'）
 *    breaks     {number[]} 等值线值（如 [100,200,300]）
 *    options.zProperty     {string}  输出线的属性名
 *  返回：FeatureCollection<MultiLineString>
 *
 *  示例：
 *    // 1. 先生成带高程值的点网格
 *    var grid = turf.pointGrid([116,39,117,40], 5, { units:'km' });
 *    grid.features.forEach(function(f) {
 *      f.properties.elevation = Math.random() * 500;  // 模拟高程
 *    });
 *    // 2. 生成等值线
 *    var lines = turf.isolines(grid, 'elevation', [100,200,300,400]);
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  8.2  turf.isobands(pointGrid, zProperty, breaks, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成等值带（填充区域，而非线）
 *  返回：FeatureCollection<MultiPolygon>
 *
 *  示例：
 *    var bands = turf.isobands(grid, 'elevation', [0,100,200,300,400,500]);
 *    // 每个 band 的 properties[zProperty] = "100-200" 等
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  8.3  turf.interpolate(points, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将离散点插值为规则网格（IDW反距离加权）
 *  参数：
 *    options.gridType   {string}  'point'|'square'|'hex'|'triangle'
 *    options.property   {string}  插值属性名
 *    options.units      {string}
 *    options.weight     {number}  IDW权重指数（默认1）
 *  返回：FeatureCollection（带插值属性的网格）
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  九、Voronoi / Delaunay（底层：d3-voronoi + RBTree）
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  原理：Fortune's Algorithm 扫描线 + 红黑树管理海滩线
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  9.1  turf.voronoi(points, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成泰森多边形（Voronoi图）
 *  参数：
 *    points   {FeatureCollection<Point>}
 *    options.bbox  {number[]}  裁剪范围 [minLng,minLat,maxLng,maxLat]
 *  返回：FeatureCollection<Polygon>
 *
 *  示例：
 *    var pts = turf.randomPoint(20, { bbox: [116,39,117,40] });
 *    var voronoi = turf.voronoi(pts, { bbox: [116,39,117,40] });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  9.2  turf.tin(points, zProperty?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：Delaunay 三角剖分（TIN）
 *  参数：
 *    zProperty  {string}  可选，用于3D TIN
 *  返回：FeatureCollection<Polygon>（三角形）
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  十、随机生成
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  10.1  turf.randomPosition(bbox?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成一个随机坐标
 *  返回：number[]  [lng, lat]
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  10.2  turf.randomPoint(count, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成随机点集合
 *  参数：
 *    count        {number}
 *    options.bbox {number[]}  限定范围
 *  返回：FeatureCollection<Point>
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  10.3  turf.randomLineString(count, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成随机线
 *  参数：
 *    options.bbox           {number[]}
 *    options.num_vertices   {number}  顶点数（默认10）
 *    options.max_length     {number}  最大段长（度）
 *    options.max_rotation   {number}  最大转角（弧度）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  10.4  turf.randomPolygon(count, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：生成随机多边形
 *  参数：
 *    options.bbox           {number[]}
 *    options.num_vertices   {number}  顶点数（默认10）
 *    options.max_radial_length {number}  最大径向长度（度）
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  十一、辅助工具
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.1  turf.getCoord(feature)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：提取点的坐标数组
 *  返回：number[]  [lng, lat]
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.2  turf.getCoords(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：提取任意几何的坐标（嵌套数组）
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.3  turf.getGeom(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：从 Feature 中提取 Geometry 对象
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.4  turf.getType(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：获取几何类型字符串
 *  返回：'Point'|'LineString'|'Polygon'|...
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.5  turf.feature(geometry, properties?, options?)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：手动包装 Geometry → Feature
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.6  turf.coordEach(geojson, callback)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：遍历所有坐标
 *  示例：
 *    turf.coordEach(fc, function(coord, coordIndex) {
 *      coord[2] = 100;  // 给所有点加高度
 *    });
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.7  turf.featureEach(geojson, callback)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：遍历所有要素
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.8  turf.flatten(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将 Multi* 几何拆分为单个几何
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.9  turf.explode(geojson)
 * ─────────────────────────────────────────────────────────────────────────
 *  作用：将任何几何拆分为独立的点
 *
 * ─────────────────────────────────────────────────────────────────────────
 *  11.10  turf.meta / turf.helpers
 * ─────────────────────────────────────────────────────────────────────────
 *  底层工具：coordReduce, geomEach, propEach, segmentEach 等
 *
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  十二、与 Cesium 配合使用
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  ─── 方法一：GeoJsonDataSource 直接加载 ───
 *
 *    var grid = turf.hexGrid([116,39,117,40], 5, { units:'km' });
 *    Cesium.GeoJsonDataSource.load(grid, {
 ...
 *  ─── 方法二：手动转换坐标 ───
 *
 *    var poly = turf.circle([116.4,39.9], 10, { units:'km' });
 *    var coords = turf.getCoords(poly);  // [[[lng,lat],...]]
 *    var positions = Cesium.Cartesian3.fromDegreesArray(
 *      coords[0].flat()   // [lng,lat,lng,lat,...]
 *    );
 *    viewer.entities.add({
 *      polygon: {
 *        hierarchy: positions,
 ...
 *  ─── 方法三：等值线渲染 ───
 *
 *    var lines = turf.isolines(grid, 'elevation', [100,200,300]);
 *    Cesium.GeoJsonDataSource.load(lines, {
 *      stroke: Cesium.Color.YELLOW,
 *      strokeWidth: 3
 *    }).then(function(ds) { viewer.dataSources.add(ds); });
 *
 *  ─── 方法四：Voronoi + 点击查询 ───
 *
 *    var voronoi = turf.voronoi(stationPoints, { bbox: cityBbox });
 *    // 点击地图 → 获取经纬度 → booleanPointInPolygon 判断属于哪个站点
 *    viewer.screenSpaceEventHandler.setInputAction(function(click) {
 *      var carto = viewer.camera.pickEllipsoid(click.position);
 *      var lngLat = Cesium.Cartographic.toDegrees(
 *        viewer.scene.globe.ellipsoid.cartesianToCartographic(carto)
 *      );
 *      voronoi.features.forEach(function(cell, i) {
 *        if (turf.booleanPointInPolygon([lngLat.longitude, lngLat.latitude], cell)) {
 *          console.log('属于站点', i);
 *        }
 *      });
 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *  1. 坐标顺序：永远是 [经度lng, 纬度lat]，不是 [lat, lng]！
 *
 *  2. 单位：默认 kilometers，可传 { units: 'miles'|'degrees'|'meters'|'radians' }
 *
 *  3. 多边形闭合：polygon 首尾坐标必须相同，否则报错
 *
 *  4. 性能瓶颈：
 *     - union/intersect/difference 底层是 JSTS（纯JS），>500个多边形会很慢
 *     - 建议：先用 bbox 粗筛，再精确运算
 *     - 大量操作考虑 Web Worker 或后端（PostGIS）
 *
 *  5. 内存：
 *     - 本文件约 400~500KB（含 JSTS）
 *     - 如只用 hexGrid，可单独 npm install @turf/hex-grid（~10KB）
 *
 *  6. 精度：
 *     - 内部使用双精度浮点，极小多边形（<1m）可能有拓扑错误
 *     - 用 turf.cleanCoords() 预处理可避免部分问题
 *
 *  7. 版本：
 *     - 本文件为完整构建版，全局变量为 turf
 *     - 模块化引入：import { hexGrid } from '@turf/turf'
 *
 *  8. 常见错误：
 *     - "coordinates must contain numbers" → 传了字符串
 *     - "Invalid GeoJSON" → 多边形未闭合 / 坐标顺序反了
 *     - union 返回 null → 两个多边形完全不相交
 *
 * ============================================================================
