...
import { Pass } from './cesiumCompat.js'
...
export default class ParticleSystem {
    ...
    refreshParticles(maxParticlesChanged, dynamic = true) {
      if (dynamic) {
        ...
        if (maxParticlesChanged) {
            ...
            if (this.particlesRendering.primitives.segments.commandToExecute) {
                this.particlesRendering.primitives.segments.commandToExecute.vertexArray = vertexArray;
            } else {
                this.particlesRendering.primitives.segments._pendingVertexArray = vertexArray;
            }
        }
    }
    ...
    applyUserInput(userInput) {
        ...
        if (userInput.maxParticles !== undefined) {
            const particlesTextureSize = Math.ceil(Math.sqrt(userInput.maxParticles));
            userInput.maxParticles = particlesTextureSize * particlesTextureSize;
            userInput.particlesTextureSize = particlesTextureSize;
        ...
        const dynamic = userInput.dynamic !== undefined ? userInput.dynamic : this.userInput.dynamic;
        this.refreshParticles(maxParticlesChanged, dynamic);
    }
    ...
    applyViewerParameters(viewerParameters) {
      ...
      this.refreshParticles(false, this.userInput.dynamic);
    }
}